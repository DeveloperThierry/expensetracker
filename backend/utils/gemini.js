...
export const generateSavingsTips = async ({ topCategories, monthlyIncome, currency = 'USD' }) => {
    const categoryText = topCategories.length > 0
        ? topCategories.map(c => `- ${c.category}: ${currency} ${c.amount.toFixed(2)} across ${c.transactionCount} transactions`)
...
export const analyzeTransactionList = async ({ transactions, currency = 'USD' }) => {
    ...
    const lines = transactions
        ...
        const prompt = `Analyze these ${transactions.length} transactions and provide a concise, helpful spending insight. Focus on patterns, notable 
Transactions:
${lines}
...
"highlight": "Single short phrase capturing the key takeaway (e.g., 'Heavy on dining', 'Stable income', 'Dinining spike')"
}`;
...
export const analyzeBudgetList = async ({ budgets, currency = 'USD' }) => {
    ...
    const prompt = `You're a personal finance assistant. Analyze each budget below and provide a one-sentence verdict for each.
